---
title: Nazo Game 攻略
date: 2025/10/06
update: 2025/10/07
description: NazoGame解题全集（含彩蛋）
categories: 
	- Puzzle
tags: 
	- Puzzle
keywords: 
	 - [NazoGame, puzzles]
cover: ../../../img/NazoGame/home.png
top_img: ../../../img/top_img.jpg
highlight_shrink: false
---

# Nazo Game

这是一个创建于2016年8月2日的在线解谜游戏，已完结。打算做第二部的开发组也跑路了。
但是这不妨碍它成为一个经典的系列谜题游戏。

{% link Nazo Game ,https://nazo.one-story.cn/ , ../../../img/link.png %}

---

## 规则

{% note info modern %}
为确保您的游戏体验 请使用PC进行游戏(Chrome最佳)
谜题涉及电脑知识,语言学,梗等等。
不会因为不懂计算机而卡关,
因为这同时也检验了您的搜索水平。
答案大小写并没有那么敏感，请安心食用
玩家交流群需先通过解谜,谜题地址放在第10关某处,每通过10关将会获得一个提示.
为了您和他人的游戏体验
请不要将答案或解题思路透露到任何平台上。
{% endnote %}

---

## 谜题正文

以下内容分关卡展开，可以自行跳转相应的关卡

### 第一关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/nazo-1`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level1.png %}
提示得很明显了吧……

{% folding 答案 %}
{% tip success %}在横线上输入`答案`即可{% endtip %}
{% endfolding %}

### 第二关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/nazo-2`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level2.png %}
同样是入门级

{% folding 答案 %}
{% tip success %}在横线上输入`入答案`即可{% endtip %}
{% endfolding %}

### 第三关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/nazo-3`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level3.png %}
横线消失了！怎么进入下一关呢？
想一想，要访问一个网站，除了让浏览器帮你跳转，还可以……

{% folding 答案 %}
{% tip success %}将网址`https://nazo.one-story.cn/nazo-3`中的`nazo-3`改为`nazo-4`即可{% endtip %}
{% note info modern %}以后的题目都是这样操作{% endnote %}
{% endfolding %}


### 第四关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/nazo-4`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level4.png %}
怎么和上一关一样啊。轻送秒掉……了吗？
{% image ../../../img/NazoGame/level5-fake.png %}
去左下角看看有多少和你一样的倒霉蛋~

那这关怎么解呢？

{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}回忆一下 "规则"{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}为什么要求用`PC`游玩呢？{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}打开`PC`端特有的`开发人员工具`看看呢{% endtip %}
<!-- endtab -->
<!-- tab 提示 4 -->
{% tip key %}查看浏览器`Console Log`（或`控制台`）{% endtip %}
{% image ../../../img/NazoGame/level4-console.png %}
<!-- endtab -->
{% endtabs %}

答案已经呼之欲出了！
{% folding 答案 %}
{% tip success %}`level-five`{% endtip %}
{% endfolding %}

### 第五关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/level-five`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level5.png %}

{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}`javascript`{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}百度/bing/google一下，你就知道~{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
在`javascript`中，无意义数字与任何东西作比较都是`false`
{% tip success %}`false`{% endtip %}
{% endfolding %}

### 第六关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/false`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level6.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}4个字母的笔画{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}小键盘{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}有两个不是字母{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`H1Z1`（大小写不敏感）{% endtip %}
{% endfolding %}

### 第七关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/h1z1`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level7.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}关注字形{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}关注对称性{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
题中字母均有且仅有竖直对称轴，按照字母表顺序排列
{% tip success %}`OTU`（大小写不敏感）{% endtip %}
{% endfolding %}

### 第八关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/otu`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level8.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}某个影视形象{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}来自官方提示`It's Bird`{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}音乐剧《It's a Bird... It's a Plane... It's Superman!》{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`superman`（大小写不敏感）{% endtip %}
{% endfolding %}

### 第九关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/superman`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level9.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}颜色{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}`开发人员工具`{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}`Element`（或者`元素`）{% endtip %}
<!-- endtab -->
<!-- tab 提示 4 -->
{% tip key %}十六进制颜色表示法：`#xxxxxx`{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
可以看到`style="background-color:#66CCFF`
{% tip success %}`66CCFF`（大小写不敏感）{% endtip %}
{% endfolding %}

### 第十关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/66ccff`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level10.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}反过来念{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}`开发人员工具`中的`Console Log`{% endtip %}
{% image ../../../img/NazoGame/level10-console.png %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}反过来写{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`ERIH`（大小写不敏感）{% endtip %}
{% endfolding %}

{% note info modern %}
溯源：这关本来的题面是`goL elosnoC uhihZ`，但是现在的知乎`Console Log`改版了，于是这一关的提示就变成了`这里本来有个提示的，但是现在没有了`
{% endnote %}

### 第十一关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/erih`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level11.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}去过`Bilibili`吗？分享过视频吗？{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}官方提示中的`XX`（可能）不止一个，但都是很正经的国外网站，其中一个在2010年1月13日撤出中国市场{% endtip %}
{% link 百度百科,https://baike.baidu.com/item/%E8%B0%B7%E6%AD%8C%E4%BA%8B%E4%BB%B6/6971355 , ../../../img/link.png %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}由于`YouTube`上的搜索结果过于离谱，这里给出原视频链接（如果你可以打开的话）{% endtip %}
{% link Video,https://www.youtube.com/watch?v=zNGQXChu9P8 , ../../../img/link.png %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`Fantastic`（大小写不敏感）{% endtip %}
{% endfolding %}

### 第十二关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/fantastic`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level12.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}学习过地理的人应该能一眼丁真吧{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}打开某在线地图{% endtip %}
{% image ../../../img/NazoGame/level12-map.png %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`Statue_of_Liberty`（大小写不敏感）{% endtip %}
{% endfolding %}


### 第十三关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/statue_of_liberty`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level13.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}这是一些单词的简写{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}2 levels back background music(BGM in short){% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}请回到那个视频{% endtip %}
<!-- endtab -->
<!-- tab 提示 4 -->
{% tip key %}看看评论区吧{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
视频的BGM是`Windows Vista`的开机音乐，内部代号为`Longhorn`（好古早的东西）
{% tip success %}`Longhorn`（大小写不敏感）{% endtip %}
{% endfolding %}


### 第十四关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/longhorn`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level14.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}官方提示：`Assembly directive`（汇编伪指令）{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}`jump $`{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
在`Assembly directive`中，`jump $`代表死循环，也就是`loop`
{% tip success %}`loop`{% endtip %}
{% endfolding %}

### 第十五关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/loop`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level15.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}谜底自在谜面中{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}人不能总是低头看大地，也需要仰望星空{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
在标签页中可以看到`answer: awesome`
{% tip success %}`awesome`{% endtip %}
{% endfolding %}

### 第十六关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/awesome`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level16.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}本题来源于某影视作品{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}《生活大爆炸》{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
《生活大爆炸》中，天才Sheldon提出了一个问题：最棒的数字是几？ 答案是唯一的。 天文学家Raj答道："5318008？" Sheldon解释道："错误答案。 最棒的数字应该是73。 这是因为，73是第21个质数，而它反过来说，37是第12个质数，而它再反过来21则是构成73的两个数字，也就是7和3相乘的结果。"
{% tip success %}`number73`{% endtip %}
{% endfolding %}

### 第十七关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/number73`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level17.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}更精确地说，应该是`撤销`{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}数据库术语{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
数据库中，`Undo`的意思是撤销
{% tip success %}`Undo`（大小写不敏感）{% endtip %}
{% endfolding %}

### 第十八关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/undo`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level18.png %}
又来一遍？不能还是`Undo`啊
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}在一般的文本编辑器中，如何实现`撤销`操作{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}快捷键{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}按下快捷键{% kbd Ctrl %} + {% kbd Z %}或者{% kbd command %} + {% kbd Z %}{% endtip %}
{% endfolding %}

### 第十九关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/brainfuck`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level19.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}再抬头看看？{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}`brainfuck`是一种编程语言（很神奇吧）{% endtip %}
{% link wiki,https://wiki.mistarg.cn/index.php/Brainfuck/Ook/shortOok , ../../../img/link.png %}
尝试自己解码？
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`she`{% endtip %}
{% endfolding %}

### 第二十关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/she`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level20.png %}
没什么好提示的，自己搜一下。
{% folding 答案 %}
{% tip success %}`moto_360`{% endtip %}
{% endfolding %}

### 第二十一关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/moto_360`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level21.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}$8 \times 8 = 64$，黑白相间的格子……{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}某种棋类游戏{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}国际象棋的棋子{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
这是国际象棋中骑士（`knight`）的行走方式
{% tip success %}`knight`{% endtip %}
{% endfolding %}

### 第二十二关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/knight`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level22.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}学过前端开发的应该知道{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}需要你调整窗口大小{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}将浏览器窗口调整到宽度$\lt 320\text{px}${% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`TOBORRM`（大小写不敏感）{% endtip %}
经查，TOBORRM 是一个Github上的账号，但是似乎没有什么贡献……
{% endfolding %}

### 第二十三关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/toborrm`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level23.png %}
没什么好提示的，百度识图即可。
{% folding 答案 %}
{% tip success %}`fsociety`{% endtip %}
{% endfolding %}

### 第二十四关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/fsociety`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level24.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}8个字母{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}和音乐相关{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}这是一首歌的简谱{% endtip %}
<!-- endtab -->
<!-- tab 提示 4 -->
{% tip key %}试着唱出来？{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`birthday`{% endtip %}
{% endfolding %}

### 第二十五关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/birthday`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level25.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}如果你见过一种有意义的乱码，你应该知道怎么做{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}这道题和用base64将一段话编码本质一样{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}将它扔到`Console Log`（或者`控制台`）中看看？{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% image ../../../img/NazoGame/level25-console.png %}
{% tip success %}`encode`{% endtip %}
如果你点击旁边的`VM1727:3`，你会发现这一大段颜文字只是调用了一个js文件……
{% folding 小彩蛋 %}
如果你的屏幕宽度过小，你会看到一行小字：`答题框被你无情的挤爆了。请在更大的屏幕上观看。`
{% image ../../../img/NazoGame/level25-stinger.png %}
{% endfolding %}
{% endfolding %}

### 第二十六关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/encode`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level26.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}十年之前~{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}如果你还留存着功能机，拿出来看看{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}功能机的键盘及使用方法{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`feelings`{% endtip %}
{% endfolding %}

### 第二十七关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/feelings`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level27.png %}
别看到二进制码就急着翻译……
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}本题与二进制无关{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}本题和$7 \times 7 = 49$有关{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}在纸上画出来？{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% image ../../../img/NazoGame/level27-answer.png %}
是吃豆人，英文是"pac-man"
{% tip success %}`pac-man`{% endtip %}
{% endfolding %}

### 第二十八关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/pac-man`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level28.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}百度搜索一下{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}这是某商业品牌，看看它的广告{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}找找它的客服电话{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`4008208820`{% endtip %}
{% endfolding %}

### 第二十九关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/4008208820`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level29.png %}
很明显，上一关的数字$+2$即可
{% folding 答案 %}
{% tip success %}`4008208822`{% endtip %}
{% endfolding %}

### 第三十关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/4008208822`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level30.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}这不是16进制{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}这是`Unicode`编码{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}网站不能输入中文，请翻译成英文{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
是“守望先锋”，英文名"Overwatch"
{% tip success %}`overwatch`{% endtip %}
{% endfolding %}

### 第三十一关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/overwatch`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level31.png %}
中国第一只企鹅的昵称？
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}某知名软件{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}QQ第一位用户的昵称{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
第一位QQ用户（马化腾）的昵称`pony`
{% tip success %}`pony`{% endtip %}
{% endfolding %}

### 第三十二关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/pony`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level32.png %}
凯撒密码？
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}回顾一下密码发展史{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}凯撒密码$\to$维吉尼亚密码{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}`PBQPXCQ`是密文，`PONY`是明文{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
解密得到`andriod`
{% tip success %}`andriod`{% endtip %}
什么，你说不对？恭喜你，你预判到了出题人的思路~
{% endfolding %}

### 第三十三关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/andriod`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level33.png %}
没什么好说的，把正确的单词填进去
{% folding 答案 %}
{% tip success %}`android`{% endtip %}
{% endfolding %}

### 第三十四关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/android`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level34.png %}
百度识图：
 - 什么干扰，我还以为是网络不好呢。
 - 跟我的识别系统说去吧！
 - 图生啊能不能放过我这一次~
{% folding 答案 %}
百度识图得到`ljn`
{% tip success %}`ljn`{% endtip %}
{% endfolding %}

### 第三十五关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/ljn`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level35.png %}
在互联网上找到自己？
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}
笑话一则：某网络小白与一网友发生冲突，大放厥词曰：”我已经找到你的IP地址了，等着被DDOS吧！“对方等了很久，无事发生。经询问得知他攻击的IP是`192.168.0.1`。
{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}很明显网址没有`.`这个字符，寻找一些替代品吧{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
你可以使用`localhost`、`127.0.0.1`或者在`ipconfig`中获取的`192.168.x.x`访问你自己的设备。很多开发者在本地测试就需要用到这些地址（比如本文的作者）
{% tip success %}`localhost`{% endtip %}
{% endfolding %}

### 第三十六关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/localhost`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level36.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}这个人是谁，搜一下{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}Notch's death{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
Minecraft老梗：当你使用Notch这个名字进入Minecraft并死亡，会掉落一个苹果`apple`
{% tip success %}`apple`{% endtip %}
{% endfolding %}

### 第三十七关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/apple`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level37.png %}
按照顺序将单词接上去即可
{% folding 答案 %}
{% tip success %}`supercalifragilisticexpialidocious`{% endtip %}
这真的是一个单词哦
{% link 百度百科,https://baike.baidu.com/item/Supercalifragilisticexpialidocious/7507391 , ../../../img/link.png %}
{% endfolding %}

### 第三十八关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/supercalifragilisticexpialidocious`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level38.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}压缩包图片？{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}把图片下载下来，把后缀名改为压缩包`zip`{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}打开里面的文件以及里面的网址{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
自己调用自己，这就是`递归`（`recursion`）
{% tip success %}`recursion`{% endtip %}
{% endfolding %}

### 第三十九关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/recursion`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level39.png %}
不是我认识的图，直接扔给百度识图
{% folding 答案 %}
{% tip success %}`trollface`{% endtip %}
在《暴走漫画》中有他。
{% endfolding %}

### 第四十关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/trollface`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level40.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}找规律{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}这是某个规律性事物的局部{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}元素周期表{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
第17号元素`氯`，英文`chlorine`
{% tip success %}`chlorine`{% endtip %}
{% endfolding %}

### 第四十一关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/chlorine`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level41.png %}
谷歌和华为的合作产品？
{% folding 答案 %}
这个合作项目叫做`Google Nexus 6P`
{% tip success %}`nexus_6p`{% endtip %}
{% endfolding %}

### 第四十二关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/nexus_6p`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level42.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}小学时候信息课的回忆{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}`Python`中的`turtle`库有相同的用法{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}`logo`语言的海龟画图{% endtip %}
<!-- endtab -->
<!-- tab 提示 4 -->
{% tip key %}海龟+手里剑？忍者神龟！{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
`忍者神龟`，英文`ninja_turtle`
{% tip success %}`ninja_turtle`{% endtip %}
{% endfolding %}

### 第四十三关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/ninja_turtle`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level43.png %}
黑？把图片拉下来，亮度和对比度拉满！
{% folding 答案 %}
{% image ../../../img/NazoGame/level43-answer.png %}
{% tip success %}`Dee.html`{% endtip %}
{% endfolding %}

### 第四十四关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/Dee.html`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level44.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}某种语言{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}某种旗帜{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}航海信号旗{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`poker`{% endtip %}
{% endfolding %}

### 第四十五关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/poker`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level45.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}百度一下？{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}某种语言{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}`Leet`语言，自行翻译一下{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
翻译结果：
```
This is an example of leet at its finest
The answer is : sigma
```
{% tip success %}`sigma`{% endtip %}
{% endfolding %}

### 第四十六关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/sigma`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level46.png %}
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}`索尼克`（`sonic`）{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}是梗图，搜一下{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`sanic`{% endtip %}
{% endfolding %}

### 第四十七关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/sanic`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level47.png %}
百度识图？
{% folding 答案 %}
`Excel2000`的彩蛋，具体见链接：
{% link stinger,https://zhuanlan.zhihu.com/p/398713790 , ../../../img/link.png %}
话说这种彩蛋是怎么被发现的啊啊啊啊啊（震惊）
{% tip success %}`2000wc`{% endtip %}
{% endfolding %}

### 第四十八关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/2000wc`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level48.png %}
这东西怎么一秒跳一次。
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}和时间相关{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}它即将达到它的上限{% endtip %}
<!-- endtab -->
<!-- tab 提示 3 -->
{% tip key %}来自`Unix`操作系统{% endtip %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
`时间戳`，英文`time stamp`
{% tip success %}`time_stamp`{% endtip %}
{% endfolding %}

### 第四十九关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/time_stamp`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level49.png %}
这是整个游戏操作最多的一关（如果你没有`Sourcetree`这类`git`管理软件）。直接上结论（请提前安装好`Git`）：
 - 下载图片，将后缀名改为`.zip`
 - 解压到合适的地方
 - 发现`answer.txt`，打开发现被骗
 - 使用`git log --stat`查看历史记录，发现有3次提交
 - 使用`git reset --hard 提交序列号`回滚提交。
 - 重复2次得到正确的`answer.txt`
但是，如果你安装了`Sourcetree`，可以直接秒杀这一题：
{% image ../../../img/NazoGame/level49-answer.png %}
{% folding 答案 %}
{% tip success %}`gitisawesome`{% endtip %}
这操作确实"awesome"
{% endfolding %}

### 第五十关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/gitisawesome`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level50.png %}
终于有官方提示了！
{% tabs 提示, -1 %}
<!-- tab 提示 1 -->
{% tip key %}官方提示：音乐相关{% endtip %}
<!-- endtab -->
<!-- tab 提示 2 -->
{% tip key %}去某知名音乐网站上搜索一下{% endtip %}
{% link /javascript,https://music.163.com/#/song?id=455920556 , ../../../img/link.png %}
<!-- endtab -->
{% endtabs %}

{% folding 答案 %}
{% tip success %}`w1-2`{% endtip %}
{% endfolding %}

### 第五十一关
{% folding 关卡地址 %}
{% note info modern %}`https://nazo.one-story.cn/w1-2`{% endnote %}
{% endfolding %}
{% image ../../../img/NazoGame/level51.png %}
__恭喜通关！__

## 彩蛋

### 交流群
我们回到第十关、第二十关、第三十关、第四十关、第五十关。
{% image ../../../img/NazoGame/level10.png %}
{% image ../../../img/NazoGame/level20.png %}
{% image ../../../img/NazoGame/level30.png %}
{% image ../../../img/NazoGame/level40.png %}
{% image ../../../img/NazoGame/level50.png %}
提取提示中`[]`框住的部分
 - `https://nazo.one-story.cn/q.html`
 - 有一个明显不是字母!
 - 图片背景并非全部纯白
 - lsb!
 - 看似纯白非纯白,基本隐写不难筛,待我拿到函数式,谜题自然解开来.
首先进入给出的网址：
{% link 交流群网址,https://nazo.one-story.cn/q.html , ../../../img/link.png %}
{% image ../../../img/NazoGame/QQgroup.png %}
很明显左下角的`0`不是字母
`lsb`指的是`最低有效位`（但我不知道有啥用）
这种类隐写术直接上“刑具”：`Stegsolve`（安装方法自行上网搜索）
将图片导入后点击{% kbd < %}，转到`Gray bits`
出现了！$y = 2x + 1$！
按照$x$递增的顺序读取$x,y$坐标，得到`EXCELLENT`，输入下方得到群号`216526508`

{% folding 歪门邪道 %}
将这张图喂给deepseek，让它分析，它会直接给出$y = 2x + 1$这个结果，原因不详，初步推测文件中直接写入了$y = 2x + 1$
科技的力量是无穷的（误
{% endfolding %}

### 报告中的彩蛋

在`PC`打开`报告`，一直下拉，背景会越来越黑，但是有一种直觉告诉你：这个报告有东西瞒着我。
报告中强调“请使用PC寻找”，那么顺理成章地，我们打开`Console Log`,发现了一堆数字。
根据传统艺能，我们找到调用的`js`文件，
{% image ../../../img/NazoGame/stinger-console.png %}
发现“罪魁祸首”：
{% image ../../../img/NazoGame/stinger-script.png %}
发现彩蛋网址`www.thenazo.com`
{% tip warning faa-horizontal animated %}
警告！请不要在公共场合打开该网址，否则造成的后果与本站无关！
警告！请不要在公共场合打开该网址，否则造成的后果与本站无关！
警告！请不要在公共场合打开该网址，否则造成的后果与本站无关！
以下是该域名的`whois`数据，请再次考虑是否打开该网址：
{% image ../../../img/NazoGame/stinger-whois.png %}
{% endtip %}
猜测这个网址应该是开发组对下一个`NazoGame`项目的规划，但是似乎已经落灰了~

# 全文完
完结撒花，写了我整整两天。